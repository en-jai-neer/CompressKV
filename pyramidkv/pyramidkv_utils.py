...
class StreamingLLMKVCluster():
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups):
        ...
        if q_len < self.max_capacity_prompt:
            ...
            k_past_compress = key_states[:, :, :-self.window_size, :].gather(dim = 2, index = indices) #
            v_past_compress = value_states[:, :, :-self.window_size, :].gather(dim = 2, index = indices) 
            k_cur = key_states[:, :, -self.window_size:, :] # Cur context
            v_cur = value_states[:, :, -self.window_size:, :]
            key_states = torch.cat([k_past_compress, k_cur], dim = 2)
            value_states = torch.cat([v_past_compress, v_cur], dim = 2)
            return key_states, value_states
...
def init_Compress(self):
    ...
    self.kv_cluster = CompressKVCluster(
        window_size = self.config.window_size, 
        max_capacity_prompt = self.config.max_capacity_prompt, 
        kernel_size = self.config.kernel_size,
        pooling = self.config.pooling
...
class CompressKVCluster():
    ...
    def compress2d_loop(self, x,start, end, threshold):
        x = self.slice2d(x, start, end)
        while threshold < x.size(2):
            x1 = self.slice2d(x,0,threshold)
            x2 = self.slice2d(x,threshold,x.size(2))
            x1_c = self.compress2d(x1)
            x = torch.cat((x1_c,x2), dim = 2)
        return x
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups):
        ...
        if q_len < self.max_capacity_prompt:
            ...
            k_context = key_states[:, :, -self.context_tokens:, :]

            v_sink = value_states[:, :, :self.sink_tokens, :]
            v_compress = value_states[:, :, self.sink_tokens : -self.context_tokens, :]
            v_compress = self.compress2d_loop(v_compress, self.sink_tokens, seq_len - self.context_tokens, threshold)
            v_context = value_states[:, :, -self.context_tokens:, :]

            key_states = torch.cat((k_sink, k_compress, k_context), dim = 2)
            value_states = torch.cat((v_sink, v_compress, v_context), dim = 2)
            return key_states, value_states
